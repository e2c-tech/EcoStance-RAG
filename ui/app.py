...
# --- Beta Agent Functions ---
def chat_with_agent(session_id, message, knowledge_base=None):
    """Send message to beta AI agent"""
    try:
        payload = {
        ...
        response = requests.post(f"{BACKEND_URL}/beta/agent/chat", json=payload)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def get_agent_history(session_id):
    ...
    try:
        response = requests.get(f"{BACKEND_URL}/beta/agent/history/{session_id}")
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}
...
def reset_agent_conversation(session_id):
    ...
    try:
        response = requests.post(f"{BACKEND_URL}/beta/agent/reset/{session_id}")
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": str(e)}
...
tab1, tab2, tab3, tab4 = st.tabs(["💬 Chat with Docs", "📁 Manage Files", "🗃️ Chat with Database", "🧪 AI Agent (Beta)"])
...
with tab4:
    st.header("🧪 QuickShip AI Agent (Beta)")
    
    # Beta warning banner
    st.warning("⚠️ **Beta Feature**: This is an experimental AI agent for QuickShip logistics. It can help you track shipments, check delivery status, and answer questions about your orders.")
    
    # Show selected knowledge base
    if selected_kb:
        st.info(f"📚 **Knowledge Base**: {selected_kb} (Agent will search this KB for document queries)")
    else:
        st.info("📚 **Knowledge Base**: None selected (Agent can only query shipment database)")
    ...
    with col2:
        if st.button("🔄 New Conversation", use_container_width=True):
            reset_result = reset_agent_conversation(st.session_state.agent_session_id)
    ...
    # Chat input
    if prompt := st.chat_input("Ask about your shipment... (e.g., 'Where is my order?', 'Track QS250001')"):
        # Add user message
        st.session_state.agent_messages.append({"role": "user", "content": prompt})
        
        # Get agent response with selected KB
        with st.spinner("🤔 Agent is thinking..."):
            result = chat_with_agent(
                st.session_state.agent_session_id, 
                prompt,
                knowledge_base=selected_kb  # Pass selected KB from sidebar
            )
    ...
    # Help section
    with st.expander("💡 How to use the AI Agent"):
        st.markdown("""
        **The AI Agent can help you with:**
        
        **📦 Shipment Tracking (Database Queries):**
        - Track shipments by ID (e.g., "Track QS250001")
        - Search shipments by phone or email
        - Check delivery estimates
        - View payment status
        - Check complaints
        
        **📚 Company Information (Knowledge Base Queries):**
        - Search knowledge base documents
        - Answer questions about policies, procedures, FAQs
        ...
        **⚠️ Important Notes:**
        ...
        1. **Knowledge Base Selection:**
           - Select a KB from the sidebar before asking document questions
           - The agent will search only the selected KB
           - If no KB is selected, only shipment queries will work
        
        2. **Knowledge Base Content:**
           - The `logistics-demo` KB now contains QuickShip policies, rates, and procedures
           - You can ask about shipping rates, delivery times, pickup services, and more
           - The agent will search the KB and provide accurate answers from the documents
        
        3. **How to Add Content:**
           - Go to sidebar → "Add New Document"
           - Upload your policy/procedure documents (PDF, DOCX, TXT)
           - Wait for processing to complete
           - Then ask the agent about that content
        
        4. **Hybrid Queries:**
           - The agent can combine both database and KB searches
           - Example: "My order is delayed, what are my options?" 
             → Checks shipment status + searches KB for delay policy
        
        **💡 Tip:** If the agent says "I don't know" for a policy question, it means that information 
        isn't in your knowledge base yet. Upload the relevant documents first!
        """)
