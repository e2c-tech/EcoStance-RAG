...
export default function DatabaseChatPage() {
  ...
  const handleSQLiteUpload = async (file: File, isQuickConnect: boolean = false) => {
    try {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-text-secondary mt-1">Connect to databases and query using natural language</p>
      </div>
          ...
          <Card className="p-4 bg-surface border-border">
            ...
            {showQuickConnect && (
              <div className="mb-4 p-3 bg-background rounded-lg space-y-2">
                <p className="text-xs text-text-secondary mb-2">Quick Connect (not saved)</p>
                <select
                  ...
                  onChange={(e) => setQuickConnectForm({ ...quickConnectForm, db_type: e.target.value })}
                >
                ...
                {quickConnectForm.db_type !== 'sqlite' ? (
                  <>
                    <input
                      ...
                      onChange={(e) => setQuickConnectForm({ ...quickConnectForm, host: e.target.value })}
                    />
                    <input
                      type="text"
                      placeholder="Port"
                      ...
                      onChange={(e) => setQuickConnectForm({ ...quickConnectForm, port: e.target.value })}
                    />
                    <input
                      type="text"
                      ...
                      onChange={(e) => setQuickConnectForm({ ...quickConnectForm, username: e.target.value })}
                    />
                    <input
                      type="password"
                      ...
                      onChange={(e) => setQuickConnectForm({ ...quickConnectForm, password: e.target.value })}
                    />
                    <input
                      type="text"
                      ...
                      onChange={(e) => setQuickConnectForm({ ...quickConnectForm, database: e.target.value })}
                    />
                  </>
                ) : (
                  <div className="space-y-2">
                    <label className="text-xs text-text-secondary">Upload SQLite Database File</label>
                    <input
                      type="file"
                    ...
                    <input
                      ...
                      onChange={(e) => setQuickConnectForm({ ...quickConnectForm, db_path: e.target.value })}
                    />
                  </div>
                )}
                <div className="flex gap-2">
                  <Button size="sm" onClick={handleQuickConnect} disabled={loading}>
                    <Play className="w-4 h-4 mr-1" />
                    Connect
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => setShowQuickConnect(false)}>
                    Cancel
                  </Button>
                </div>
              </div>
            ...
            {showConnectionForm && (
              <div className="mb-4 p-3 bg-background rounded-lg space-y-2">
                <p className="text-xs text-text-secondary mb-2">
                  {editingConnection ? `Edit Connection: ${editingConnection}` : 'Save Connection'}
                </p>
                <input
                  type="text"
                ...
                <div className="flex gap-2">
                  <Button size="sm" onClick={handleSaveConnection} disabled={loading || !connectionForm.name.trim()}>
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex-1 min-w-0 cursor-pointer" onClick={() => handleConnect(conn.name)}>
                      <div className="font-medium text-sm text-text truncate">{conn.name}</div>
                      <div className="text-xs text-text-secondary truncate" title={conn.database}>
                        {conn.type} - {conn.type === 'sqlite' ? conn.database.split('/').pop() || conn.database : conn.database}
                      </div>
                    </div>
                  ...
                  {schema && schema.tables && schema.tables.length > 0 ? (
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {schema.tables.map((table) => (
                        <div key={table.name} className="text-xs border-l-2 border-primary pl-3">
                          <div className="font-semibold text-text mb-1">{table.name}</div>
                          <div className="space-y-0.5">
                            {table.columns?.map((col) => (
                              <div key={col.name} className="text-text-secondary flex items-center gap-2">
                                <span className="font-mono">{col.name}</span>
                                <span className="text-primary/60 text-[10px]">{col.type}</span>
                              </div>
                            ))}
                          </div>
                        </div>
        ...
        <div className="lg:col-span-2 flex flex-col h-[calc(100vh-12rem)]">
          {!isConnected ? (
            <Card className="p-8 text-center bg-surface border-border">
              ...
              <h3 className="text-lg font-medium text-text mb-2">No Database Connected</h3>
              <p className="text-text-secondary">Select or create a connection to get started</p>
            </Card>
          ) : (
            <>
              {/* Chat History */}
                        ...
                        {msg.data.rows && msg.data.rows.length > 0 ? (
                          <div className="overflow-x-auto">
                            <div className="mb-2 text-xs text-text-secondary">
                              {msg.data.rows.length} row{msg.data.rows.length !== 1 ? 's' : ''}
                            </div>
                            <table className="w-full text-xs border border-border">
                              <thead className="bg-background">
                                <tr>
                                  {Object.keys(msg.data.rows[0]).map((col) => (
                                    <th key={col} className="px-3 py-1 text-left font-medium text-text border-b border-border">
                                      {col}
                                    </th>
                                  ))}
                                </tr>
                                ...
                                {msg.data.rows.map((row, idx) => (
                                  <tr key={idx} className="hover:bg-surface-hover">
                                    {Object.values(row).map((val, i) => (
                                      <td key={i} className="px-3 py-1 text-text">
                          ...
                          <p className="text-xs text-text-secondary">
                            {msg.data.message || `Query executed. ${msg.data.rows_affected || 0} rows affected.`}
                          </p>
                        )}
                      </Card>
                    )}
              ...
              <Card className="p-4 bg-surface border-border">
                <div className="flex gap-2">
                  ...
                  />
                  <Button onClick={handleGenerateQuery} disabled={loading || !question.trim()}>
